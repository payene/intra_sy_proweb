...
class AnimatriceController extends Controller
{
    ...
    public function createAction(Request $request)
    {
        ...
        if ($form->isValid()) {
            ...
            $anim = $em->getRepository('LooninsSuiviBundle:Animatrice')->findOneBy(['login'=> $entity->getLogin(),'del'=>0]);
            if(empty($anim)){
                $entity->setDel(0);
                $em->persist($entity);
                $em->flush();
                $this->get('session')->getFlashBag()->add('info', 'Animatrice enregistrée avec success');
